...
- LINE の user_id は個人識別子として扱う。生の user_id をそのままログに出さない。用途で以下の 2 方式を使い分ける（Issue #60）:
    - **ログ出力（ランタイムの `logger.info` / `logger.warning` など）**: 先頭 8 文字の prefix（`user_id[:8]`）。既存 `src/services/session.py`、`posts.py`、`usage_stats.py`、`coupons.py`、`prizes.py`、`monthly_report.py` などが採用済み。`src/services/usage_stats.py` のモジュール docstring にも既に方針が明記されている。
    - **永続化（統計 / 監査 / スポンサー配信履歴など、JSON に長期保存するフィールド）**: SHA256 ハッシュ（hex）。既存 `src/services/sponsored.py::_hash_user_id` が該当。復元不能な識別子として扱う。
    - コード側の実装統一（共通ラッパー `hash_uid()` の導入など）は Issue #60 対応 PR で行う。本規約は「どちらが標準か」を確定させるためのポリシー。

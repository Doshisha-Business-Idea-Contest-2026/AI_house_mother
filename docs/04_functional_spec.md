...
- **push_text / push_flex 経由の非同期通知**（保護者連携完了の学生側通知、月次サマリーの scheduled push、`handle_want_to_do` の Gemini フォールバックなど）: reply_token が使えず「次に何をすべきか」がユーザー側の履歴で完全に流れ落ちるため、reply 系より優先的に QR を付ける

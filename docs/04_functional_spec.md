...
| `handlers/student.py::handle_want_events` / `handle_want_students` の push_flex カルーセル | `friendly` |
...
- **friendly**: `handlers/follow.py::handle_follow`（welcome）、`handlers/student.py` の `start_want_to_do_menu` / `handle_want_events` / `handle_want_students` / `handle_activity_detail` / `handle_activity_participated` / `start_life_consultation` / `handle_life_consultation`（ハブ・緊急定型・「少し考えます」・Gemini 応答）、`handlers/parent.py::_reply_report_for`。
...
- **登録済みユーザー**: `templates/flex/profile_view.py::build_profile_view_bubble` で自分の情報を Flex bubble として返す。Quick Reply `profile_view_quick_reply()` は `✏️ 編集する`（postback: `menu:profile_start`）と `🏠 メインメニュー`（postback: `menu:main`）の 2 項目。sender は `system`。
...
[bot] （Gemini に地域データ + 先輩投稿 + 匿名化した学生投稿を渡す）
      ...
      37 度台の発熱なら、まずは近くの内科クリニックを受診するのが安心です。

      ・今出川周辺には徒歩圏の内科が複数あります
      ・先輩の投稿より: 「土曜午前も空いていて待ち時間が短かった」

      症状が続く場合や高熱のときは迷わず医療機関へ。
      🆘 緊急の場合は #7119（京都府救急安心センター）へ。
```

回答は**結論を先に 1 文**、候補・注意点を行頭「・」の箇条書き、最後に締め（誘導・注記）という構造化プレーンテキストで返す（詳細は「回答書式」参照）。
...
LINE は Markdown をレンダリングしないため、装飾記号を使わず構造化プレーンテキストで整える。

1. Gemini プロンプトで出力構造を規定: 結論先出し → 行頭「・」の箇条書き → 締め、ブロック間は空行 1 つ。`-`/`*`/`#`/`**` 等の Markdown 記号は使わせない。
2. 送信前に整形ヘルパー `src/utils/text_format.py` で Gemini 本文の Markdown 痕跡を除去（`- `/`* ` の行頭を「・」へ、`**`/`` ` `` マーカーを除去）し、3 連以上の改行を 2 に正規化する。
3. `ZERO_CONTEXT_DISCLAIMER` / 本文 / `MEDICAL_FOLLOWUP` を空行 1 つで連結し、**単一のテキストバブル**で送る（会話感を維持。Flex・複数バブルは採用しない）。
4. 緊急定型応答（`_EMERGENCY_*`）は既に整形済みのため本整形の対象外。「詳しく聞く」等ほかの Gemini 回答への横展開は将来検討。
...
seed の store / area レコードを回答に含める場合は、Bot 応答に必ず情報鮮度注記を付ける。生活相談は単一テキストバブル運用（本節上部・§4.4「回答書式」）なので、Flex 側の対応は同節ではなくやりたいこと相談（§4.3）側で扱う。

- **生活相談（本文中で強制）**: プロンプト側で `_summarise_stores` / `_summarise_areas` が各 seed レコード末尾に `[情報鮮度: ...]` を付加する。Gemini はその値を抜き出して回答本文の末尾に `※ (値)` の形で 1 文添える。実装は `src/services/prompts.py` の `build_life_consultation_prompt`（【回答時の注意】節）。
...
- `post.share_parent` の選択直後、全項目が揃った時点で **1 回だけ** Gemini を呼ぶ（`show_loading` で Loading Indicator を表示中に実行し、確認カードは push で後送）。詳細は `docs/06_ai_spec.md §4.5`。
...
- **フォールバック（フローを止めない）**: LLM 失敗・空応答・JSON パース失敗・`GEMINI_MOCK_MODE` の場合、title は `summary` の冒頭 40 文字、period は `period_raw` をそのまま採用する。

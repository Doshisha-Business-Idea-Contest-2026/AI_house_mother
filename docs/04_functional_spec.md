...
seed の store / area レコードを回答に含める場合は、Bot 応答（テキスト末尾または Flex Message フッター）に必ず情報鮮度注記を付ける。

- Store: レコードの `data_freshness_note` フィールドをそのまま挿入（例: `※2026-07 時点の情報です。営業状況は変わっている可能性があります。`）
- Area: レコードの `last_verified_at` を参照し「◯年◯月時点の情報」等を付記
- Flex Message で店舗情報を返す場合は、末尾 body に注記テキスト用 box を配置する（実装場所は `src/templates/flex/` 各テンプレ）

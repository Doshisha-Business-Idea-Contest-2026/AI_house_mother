...
**「参加した」選択後**: 経験投稿フロー（FR-S6）のカテゴリ選択に遷移する。学生の入力が上書きされないよう、活動側フィールド（カテゴリ・要約・場所・時期など）は自動で埋めず、参照した活動タイトルだけを session context の `source_activity_title` に保持する。post ウィザードの最終確認カードに「🔗 参照した活動: 「◯◯」」の 1 行を先頭表示し、どの提案から起票したかを学生に見せる（本文の AI 生成が活動タイトルを参照する拡張は今後の展望）。
...
[bot] 熱っぽいんですね😢 それはつらいですよね。
      近くの病院、一緒に探しましょう🏥

      37 度台なら、まず近くの内科クリニックの受診が安心です✨
      ...
      🗣️ 先輩の体験から
      土曜の午前も空いていて待ち時間が短かったそう

      💊 症状が続くときは無理せず受診してくださいね
      🆘 緊急なら #7119（京都府救急安心センター）へ
```

回答は Gemini から `empathy`（共感）/ `answer`（本文）/ `closing`（締め）の JSON で受け取り、空行 1 つで連結して**単一のテキストメッセージ**として送る。`empathy` は相談内容に感情（つらさ・不安・体調）が読み取れるときだけ入り、事務的な質問では省略される。本文（`answer`）は「💡 AI からのアドバイス」と「🗣️ 先輩の体験から」の見出し付き 2 セクション構成で、後者は先輩投稿・学生投稿に該当があるときだけ表示する（詳細は「回答書式」参照）。
...
LINE は Markdown をレンダリングしないため、装飾記号を使わず構造化プレーンテキストで整える。回答は共感 / 本文 / 締めを空行 1 つで連結し、`services/line_reply.push_text` で**単一のテキストメッセージ**として送る。

1. Gemini プロンプトで JSON 出力構造を規定: `empathy`（共感の 1〜2 文、感情が読み取れるときだけ。絵文字 1〜2 個）/ `answer`（結論先出し → 「💡 AI からのアドバイス」見出し + 行頭絵文字アンカーの箇条書き → 先輩投稿・学生投稿がある場合のみ「🗣️ 先輩の体験から」見出し + 引用。「・」は使わない）/ `closing`（行頭絵文字 + 気遣い + 誘導）。親しみやすさ重視で絵文字を程よく使う（ブロックごとに 1〜3 個目安）。`-`/`*`/`#`/`**` 等の Markdown 記号は使わせない。
...
3. 「共感 → 本文 → 締め」の順に `text_format.join_blocks` で空行 1 つ区切りに連結する。空フィールドは自動的に脱落する（`empathy` が空なら本文から始まる）。Zero-context 時は先頭に `ZERO_CONTEXT_DISCLAIMER`、医療系は末尾に `MEDICAL_FOLLOWUP` を連結する（組み立て順は `docs/06_ai_spec.md §5.3.6`）。Quick Reply はこの単一メッセージに付ける。
4. **単一メッセージ採用の判断**: 一時期「共感 / 本文 / 締め」を最大 3 吹き出しに分割していたが、`answer` の絵文字リッチ化（見出し + 行頭絵文字アンカー）で 1 メッセージ内でも十分に読みやすく温かみが出たため、吹き出し分割は不要と判断して単一テキストメッセージへ戻した（Flex は引き続き不採用。会話感を損なうため）。
5. 緊急定型応答（`_EMERGENCY_*`）は既に整形済みのため本整形の対象外。「詳しく聞く」等ほかの Gemini 回答への横展開は将来検討。
...
2. メッセージ**先頭**に `ZERO_CONTEXT_DISCLAIMER`（`docs/06_ai_spec.md §5.3.4`）を連結する（この場合 `empathy` は省略）
3. `context_search.detect_medical_intent(user_message)` が真なら**末尾**に `MEDICAL_FOLLOWUP` を連結（NFR-Truth-3）
4. Quick Reply には「🏠 メインメニュー」と「🚫 相談を終える」を維持
5. `zero_context: true` を journald にログ（§3.3）

**実在情報鮮度注記**（NFR-Truth-4 対応）:

seed の store / area レコードを回答に含める場合は、Bot 応答に必ず情報鮮度注記を付ける。生活相談は単一テキストメッセージ運用（本節上部・§4.4「回答書式」）なので、Flex 側の対応は同節ではなくやりたいこと相談（§4.3）側で扱う。
...
**投稿本文への PII 混入注意（Issue #59）**: 投稿本文（`summary` / `learned` / `regret` / `advice`）は、投稿者以外を特定できる情報を含んではならない。SECI モデルの継承 context として他学生の生活相談 Gemini プロンプトに **匿名化した body がそのまま渡る**（`docs/06_ai_spec.md §4.2` 準拠、`_CONTEXT_FIELDS` allow-list に `title` / `body` / `area` / `category` / `created_at` が含まれる）ため、本文中に友人・お店の人・先生など第三者の氏名や連絡先を書くと、匿名化フィルタを素通ししてしまう。MVP では文言による名前検出は行わず（誤検知回避）、`post.summary` プロンプトに「他の人の氏名は書かないでね（他の学生の相談に参考として匿名で共有されます）」の 1 行 UI 警告を出して事前抑止する。実装: `src/handlers/student.py::_POST_ENTRY_PROMPTS`。
...
**保護者側の入力正規化**（受入時、`src/handlers/parent.py::_normalize_code`）:

保護者が iOS/Android IME で送るコードは半角統一されていないことが多いため、`_is_valid_format` に照合する前に以下の正規化を適用する。**正準文字集合（`CODE_ALPHABET`）そのものは変更しない**。

1. `unicodedata.normalize("NFKC", text)` で全角英数字（`Ａ３Ｆ７Ｋ９`）を ASCII に折りたたむ
2. ASCII 空白と半角ハイフンを除去し、視覚的な区切り入力（`A3F-7K9`、`A 3 F 7 K 9`）を吸収
3. `.strip().upper()` で前後空白除去と大文字化

正規化後の文字列が `CODE_ALPHABET` に含まれない文字を残していれば `invalid_format` として通常のフローで失敗カウントする（§5.2）。
...
| ステート | Bot 発話 | ユーザー入力 |
| --- | --- | --- |
| link.code | お子さんから受け取った 6 桁のコードを入力してください（英数字。全角・ハイフン・スペースは自動で吸収される。詳細は §4.6「保護者側の入力正規化」）| テキスト（コード） |
...
やりたいこと提案のカード復元情報は `session_activities.json` に 30 分だけ保存し、会話セッションには
状態を残さない。
...
| 生活相談 | `生活相談` / `💬 生活相談` | session 全リセット → 生活相談開始（学生） |
| 経験投稿 | `投稿` / `✏️ 経験を投稿` | session 全リセット → 経験投稿開始（学生） |
| 招待 | `招待` / `👨‍👩‍👧 保護者連携` | session 全リセット → 招待コード発行（学生） |
...
| 2026-07-10 | §4.3「参加した」選択後の遷移仕様を明確化: pre-fill（活動フィールド自動埋め）を廃止し、参照した活動タイトルのみ `source_activity_title` として session に保持、post ウィザードの最終確認カードに「🔗 参照した活動: 「◯◯」」の 1 行を表示（PR #82 の docs-first 追随、実装との齟齬解消） | kmch4n |
| 2026-07-10 | §4.6「保護者側の入力正規化」節を新設し、§5.2 の link.code プロンプトに全角・ハイフン・スペース自動吸収の注記を追加（PR #87、招待コード NFKC 正規化仕様の docs-first 追随） | kmch4n |
| 2026-07-11 | §4.5 に「投稿本文への PII 混入注意」段落を新設: SECI モデルの継承 context として body が他学生の Gemini プロンプトに匿名化のまま渡るため、`post.summary` プロンプトに氏名を書かない旨の 1 行 UI 警告を出す方針を明記（Issue #59 の docs-first） | kmch4n |

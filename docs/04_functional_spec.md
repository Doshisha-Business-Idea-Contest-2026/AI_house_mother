...
[bot 吹き出し1・共感] 熱っぽいんですね😢 それはつらいですよね。
      近くの病院、一緒に探しましょう🏥

[bot 吹き出し2・本文] 37 度台なら、まず近くの内科クリニックの受診が安心です✨
...
[bot 吹き出し3・締め] 💊 症状が続くときは無理せず受診してくださいね
      🆘 緊急なら #7119（京都府救急安心センター）へ
```

回答は Gemini から `empathy`（共感）/ `answer`（本文）/ `closing`（締め）の JSON で受け取り、**最大 3 つの吹き出し**に分割して **1 回の push（通知 1 回）** でまとめて送る。`empathy` は相談内容に感情（つらさ・不安・体調）が読み取れるときだけ入り、事務的な質問では省略されて実質 2 吹き出しになる。本文（`answer`）は「💡 AI からのアドバイス」と「🗣️ 先輩の体験から」の見出し付き 2 セクション構成で、後者は先輩投稿・学生投稿に該当があるときだけ表示する（詳細は「回答書式」参照）。
...
1. Gemini プロンプトで JSON 出力構造を規定: `empathy`（共感の 1〜2 文、感情が読み取れるときだけ。絵文字 1〜2 個）/ `answer`（結論先出し → 「💡 AI からのアドバイス」見出し + 行頭絵文字アンカーの箇条書き → 先輩投稿・学生投稿がある場合のみ「🗣️ 先輩の体験から」見出し + 引用。「・」は使わない）/ `closing`（行頭絵文字 + 気遣い + 誘導）。親しみやすさ重視で各吹き出しに絵文字を程よく使う（1〜3 個目安）。`-`/`*`/`#`/`**` 等の Markdown 記号は使わせない。

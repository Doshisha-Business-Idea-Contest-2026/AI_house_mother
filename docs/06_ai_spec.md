...
| `timeout` (text / 対話 JSON) | 15 秒 | LINE Webhook 30 秒制約に対する安全マージン。`call_gemini`（テキスト経路の共通ハブ）、`answer_life_question`（JSON 応答）、`answer_activity_detail`（活動詳細を text で返す）を含む（`DEFAULT_TIMEOUT_S`） |
...
- **待機中の表示**: 通常の待機は Loading Indicator（`docs/04_functional_spec.md §3.5`、`show_loading(user_id)`）で可視化する。テキストの中間応答（「少し考えます…」等）は使わない。

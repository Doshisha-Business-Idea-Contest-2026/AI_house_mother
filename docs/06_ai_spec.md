...
【回答時の注意】
- 参照した先輩投稿・学生投稿がある場合、「先輩の体験より: ...」と 1 文添える。**投稿者を特定・推測しない**（例:「〇年生の先輩が...」等の憶測は禁止）。
...
【回答書式（JSON 構造化出力）】
以下 3 キーを持つ JSON オブジェクトだけを出力する（前後に地の文やコードフェンスを付けない）。回答は「共感 → 本文 → 締め」の順に最大 3 つの LINE 吹き出しへ割り付けられる。
- "empathy": 共感・受け止めの 1 文（絵文字は 0〜1 個）。つらさ・不安・体調など感情が読み取れる相談のときだけ書く。事務的な質問（店舗・手続き等）では空文字 "" にする。
- "answer": 回答本体。冒頭 1 文で結論・要点を先に述べ（結論先出し）、候補・手順・注意点が複数あるときは行頭「・」の箇条書きにする。実在の店舗・施設を挙げた場合は上記の [情報鮮度: ...] の値を「※ (値)」の形で末尾に添える。
- "closing": 気遣いの一言と必要な誘導（受診・#7119 等）。短く 1〜2 文。無ければ空文字 "" でよい。
- Markdown 記号（-, *, #, **）は使わない。LINE は装飾を表示せず記号がそのまま残る。
- empathy / answer / closing を合計して 300 字以内に収める。

出力例:
{"empathy": "熱っぽいんですね、それはつらいですね💦", "answer": "37 度台の発熱なら、まずは近くの内科クリニックの受診が安心です。\n\n・今出川周辺には徒歩圏の内科が複数あります\n・先輩の体験より: 土曜の午前も空いていて待ち時間が短かったそうです", "closing": "症状が続くときや高熱のときは、無理せず医療機関へ。🆘 緊急なら #7119 へ。"}
```

> **送信前整形**（実装注記）: Gemini の JSON 応答は `services/gemini._parse_life_json` で `empathy` / `answer` / `closing` に分解する。各フィールドを整形ヘルパー `src/utils/text_format.py` で Markdown 痕跡除去（`- `/`* ` の行頭を「・」へ、`**`/`` ` `` マーカー除去）・改行正規化したうえで、「共感 → 本文 → 締め」の順に最大 3 つの吹き出しへ割り付け、`services/line_reply.push_texts` で **1 回の push（通知 1 回）** としてまとめて送信する（詳細は `docs/04_functional_spec.md §4.4`、Zero-context 時の割り付けは §5.3.6）。
...
2. `should_add_disclaimer(result)` が真なら:
   ...
   - 先頭の吹き出しを `ZERO_CONTEXT_DISCLAIMER` にする（この場合 `empathy` は省略し、disclaimer が受け止めの役割を兼ねる）
   - `detect_medical_intent` が真なら締めの吹き出し末尾に `MEDICAL_FOLLOWUP` を連結する
3. `should_add_disclaimer(result)` が偽なら通常応答（`empathy` / `answer` / `closing` をそのまま吹き出しに割り付け。先輩投稿を引用しやすい設計）

吹き出し割り付け（最大 3 つ、空フィールドは送らない。実装は `handlers/student.handle_life_consultation`）:

| 順 | 通常時 | Zero-context 時 |
| --- | --- | --- |
| 1 | `empathy`（空なら省略） | `ZERO_CONTEXT_DISCLAIMER` |
| 2 | `answer` | `answer` |
| 3 | `closing`（医療時は `+ MEDICAL_FOLLOWUP`） | `closing`（医療時は `+ MEDICAL_FOLLOWUP`） |
...
- **待機中の表示**: 通常の待機（1〜25 秒）は Loading Indicator（`docs/04_functional_spec.md §3.6`、`show_loading(user_id)`）で可視化する。テキストの中間応答（「少し考えます…」等）は使わない。25 秒超えの追加通知だけは push_text で従来通り送る。

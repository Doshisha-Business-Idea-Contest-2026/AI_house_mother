...
- 候補・手順・注意点が複数ある場合は、行頭「・」の箇条書きで列挙する。
- Markdown 記号（`-`/`*`/`#`/`**`）は使わない。LINE は装飾表示せず記号がそのまま残るため。
- 「結論」「箇条書き」「締め（誘導・注記）」の各ブロックの間は空行 1 つで区切る。
- 全体は 300 字以内を維持する。

回答:
```

> **送信前整形**（実装注記）: 上記書式に加え、送信前に整形ヘルパー `src/utils/text_format.py` で Gemini 本文の Markdown 痕跡除去（`- `/`* ` の行頭を「・」へ、`**`/`` ` `` マーカー除去）・3 連以上の改行の正規化を行い、`ZERO_CONTEXT_DISCLAIMER` / 本文 / `MEDICAL_FOLLOWUP` を空行 1 つで連結してから単一テキストバブルで送信する（詳細は `docs/04_functional_spec.md §4.4`）。

...
### T4.16 クーポン配布実装（FR-S10、地域貢献インセンティブの実演）

**目的**: `docs/04_functional_spec.md §4.8`・`docs/05_data_model.md §4.15/§4.16` で確定した「経験投稿が 3 件たまるごとに地域店舗クーポン 3 種を Flex 配布し繰り返す」機能を実装する。「知識共有 → インセンティブ獲得 → 地域消費 → 地域活性化」の循環（`docs/00_product_context.md §10.5`）を実演で示す。引き換え・消込・ポイント台帳は持たない（見た目の配布のみ）。

**成果物**:

- **seed**: `data/seed/coupons.json`（架空の地域店舗クーポン 6〜9 件。`§4.15` スキーマ準拠。鮮度注記でなく `valid_until` を持つ）。ローテーションのため 6 件以上
- **カウント関数**: `src/services/posts.py` に `count_all(student_user_id) -> int` を新設（`count_all_shared` から share フィルタを外した全投稿数）
- **seed アクセサ**: `src/services/seed.py` の `_SEED_FILES` に `coupons` を追加、`get_coupons() -> list[dict]` を新設
- **配布ロジック**: `src/services/coupons.py` を新設。`award_if_due(user_id) -> list[dict] | None`（`count_all` → 節目判定 → 循環スライスで 3 種選定 → `data/coupon_distributions.json` に記録、`last_awarded_milestone` 単調増加で重複防止）と `select_coupons_for_milestone(milestone)`
- **Flex 表示**: `src/templates/flex/coupon_carousel.py` を新設。`build_coupon_carousel(coupons) -> dict`（3 バブル、店舗名・割引・`valid_until`、URI ボタン「お店で使う」→ `store_url`）。`activity_carousel.py` の構造を雛形に
- **結線**: `src/handlers/student.py::_finalize_post` の投稿保存直後に `award_if_due` を呼び、戻り値があれば完了 reply の後に `push_flex` で配布
- **初期化**: `scripts/init_data.py` に `coupon_distributions.json` の空スケルトン生成を追加

**完了基準**:

- [ ] 経験投稿を 3 件行うと 3 件目完了直後にクーポン 3 種のカルーセルが push される
- [ ] 6 件目・9 件目でもローテーションした別の 3 種が配布される
- [ ] 同一節目で二重配布されない（`last_awarded_milestone` チェック）
- [ ] クーポン店舗名が架空（「（架空）」明記）で、掲載テキストが seed の値と一致する
- [ ] 「お店で使う」で `store_url` が開く
- [ ] `coupon_distributions.json` に生の LINE user_id はキーとして入るが `.gitignore` 除外されている
- [ ] iOS / Android 両方でクーポン枠が崩れず表示される
- [ ] `black --check` / `ruff check` / `mypy` が通る

**優先度**: P1（`docs/02_mvp_scope.md §3.2`。逼迫時は落とせるが、地域貢献インセンティブ実演のため実装を目指す）

**推定**: 2〜3 時間

**Day 割当**: Day 4。T4.4（デモ通し）の前に実施し、シーン 3 の経験投稿後のクーポン配布を含めて通す。

### T4.17 プレゼント抽選応募実装（FR-S11、投稿インセンティブの実演）

**目的**: `docs/04_functional_spec.md §4.9`・`docs/05_data_model.md §4.17/§4.18` で確定した「経験投稿を完了するたびにくじを 1 回引き、当落演出まで見せる」等級制くじ引き機能を実装する。投稿インセンティブ（`docs/00_product_context.md §14.6`）を決勝で印象的に示す。**リスク回避を徹底**する: 実物配布なし・架空賞品・当選者 PII 非収集・実在ブランド不使用。

**成果物**:

- **seed**: `data/seed/prizes.json`（等級付き架空賞品 3 件〔1〜3 等〕。`§4.17` スキーマ準拠、`valid_until`/`note` 付き、実在ブランド名不使用）
- **seed アクセサ**: `src/services/seed.py` の `_SEED_FILES` に `prizes` を追加、`get_prizes()` を新設
- **抽選ロジック**: `src/services/prizes.py` を新設。`draw(user_id, *, force_rank=None)`（乱数で等級テーブルに従い当落・等級を決定、`force_rank` で等級固定〔1〜3 等 / `0` はずれ〕、`prize_draws.json` に PII を含めず seed・結果を記録）
- **Flex 演出**: `src/templates/flex/prize_result.py` を新設。`build_prize_result_bubble(result)`（当選=等級バッジ＋賞品名＋情報カード〔当選番号・有効期限・利用条件〕、落選=再挑戦。当選/落選とも小さく「デモ演出」注記）。専用の中央寄せレイアウト（白＋シャンパンゴールド、`size:"mega"` 不使用）
- **結線**: `src/handlers/student.py::_finalize_post` の投稿保存直後に自動でくじを 1 回引き、当落演出を `push_flex` する（抽選失敗は投稿フローを壊さない）
- **手動トリガー**: `scripts/trigger_prize_draw.py`（`trigger_coupon.py` と同型、`--list` / `--dry-run` / `--rank 1|2|3` / `--lose`）で等級を指定して確実に発火
- **初期化**: `scripts/init_data.py` に `prize_draws.json` の空スケルトン生成を追加

**完了基準**:

- [ ] `trigger_prize_draw.py <user_id> --rank 1` で 1 等の当選演出 Flex が届く
- [ ] `--lose` で落選演出が届く
- [ ] `prize_draws.json` に氏名・住所・連絡先などの **PII が保存されていない**（`{rank, prize_id, result, seed, drawn_at}` のみ）
- [ ] 賞品が架空で実在ブランド名が使われておらず、当選/落選バブルに「デモ演出」注記が入る
- [ ] 投稿を完了するたびに自動でくじが 1 回引かれ、当落が push される
- [ ] iOS / Android 両方で当選/落選バブルが崩れず表示される
- [ ] `black --check` / `ruff check` / `mypy` が通る

**優先度**: P1（`docs/02_mvp_scope.md §3.2`。逼迫時は落とせるが、投稿インセンティブ実演のため実装を目指す）

**推定**: 2〜3 時間

**Day 割当**: Day 4。T4.16（クーポン）に続けて実施し、デモの山場で手動トリガーによる当選演出を通す。
...
9. **T4.16 クーポン配布** — カルーセルをテキストでのクーポン名列挙に置換、それも困難なら配布自体を見送る（発表資料の「今後の展望」で言及のみ）
10. **T4.17 プレゼント抽選応募** — 当選演出をテキストに置換、それも困難なら見送る（発表資料の「今後の展望」で言及のみ）
...
| 2026-07-09 | T4.16 クーポン配布実装（FR-S10）を新設: coupons.json seed・count_all・coupons.py 配布ロジック・coupon_carousel Flex・_finalize_post 結線・配布履歴トラッキングの成果物と完了基準を定義。§8 スコープ縮退にクーポン配布の縮退行を追加 | kmch4n |
| 2026-07-10 | T4.17 プレゼント抽選応募実装（FR-S11）を新設: prizes.json seed・prizes.py 抽選ロジック（乱数/force_rank）・prize_result Flex・手動トリガー trigger_prize_draw.py・PII 非収集の抽選履歴の成果物と完了基準を定義。§8 スコープ縮退に抽選応募の縮退行を追加 | kmch4n |
| 2026-07-10 | T4.17 を最終設計に整合: 等級くじ引き（`force_rank` / 投稿完了時の自動抽選 `push_flex` / `--rank`）、`valid_until`・`note`・情報カード・抽選確率調整を反映（旧「応募口数 / force_result / --win」記述を修正） | kmch4n |

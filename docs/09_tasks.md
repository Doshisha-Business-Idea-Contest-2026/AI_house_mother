...
- **カード密度はバランス**: 各セクション見出しに「左アクセントバー＋太字タイトル」、本文は
  角丸カード（淡グレー地）で軽くグルーピングする。全項目を個別カード化するリッチ案は、要素数
  増加と端末差での崩れリスクを避けるため採らない。
- **配色は同志社ネイビー基調に調整**: `_CATEGORY_COLORS` のビビッド配色（event=明るい青 /
  festival=ピンク / store=オレンジ 等）を彩度を落とした落ち着いたトーンへ再設計し統一感を出す。
  `sponsored` のゴールド `#C9A227` は FR-S9 の識別要件のため維持する。

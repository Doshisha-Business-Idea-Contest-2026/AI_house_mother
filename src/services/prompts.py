...
def build_life_consultation_prompt(
    ...
    *,
    ...
    return (
        ...
        '- "empathy": 共感・受け止めの 1 文（絵文字は 0〜1 個）。つらさ・不安・体調など'
        '感情が読み取れる相談のときだけ書き、事務的な質問（店舗・手続き等）では空文字 "" にする。\n'
        '- "answer": 回答本体。冒頭 1 文で結論・要点を先に述べ（結論先出し）、候補・手順・'
        "注意点が複数あるときは行頭「・」の箇条書きにする。実在の店舗・施設を挙げた場合は"
        "上記の [情報鮮度: ...] の値を「※ (値)」の形で末尾に添える。\n"
        '- "closing": 気遣いの一言と必要な誘導（受診・#7119 等）。短く 1〜2 文。'
        '無ければ空文字 "" でよい。\n'
        "- Markdown 記号（-, *, #, **）は使わない。LINE は装飾を表示せず記号がそのまま残る。\n"
        "\n出力例:\n"
        '{"empathy": "熱っぽいんですね、それはつらいですね💦", '
        '"answer": "37 度台の発熱なら、まずは近くの内科クリニックの受診が安心です。\\n\\n'
        '・今出川周辺には徒歩圏の内科が複数あります", '
        '"closing": "症状が続くときは無理せず医療機関へ。🆘 緊急なら #7119 へ。"}'
    )

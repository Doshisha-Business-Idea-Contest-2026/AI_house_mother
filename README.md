...
```mermaid
flowchart LR
    U["👨‍🎓 学生 / 👨‍👩‍👧 保護者<br/>(LINE アプリ)"]
    L["📱 LINE Platform"]

    subgraph HOST["自宅 Ubuntu サーバ"]
        AP["🌐 Apache (HTTPS)"]
        APP["FastAPI + uvicorn<br/>handlers ・ services"]
        DB[("data/*.json")]
    end
...
- 受信: LINE Platform から Webhook を Apache が受け、`localhost:8084` の FastAPI へ ProxyPass。
- 応答: `reply_token` の即時応答と、時間のかかる処理向けの `push` の 2 経路。
- 保存: プロフィール・投稿・セッションなどは JSON ファイルに `fcntl` ロック付きで永続化。

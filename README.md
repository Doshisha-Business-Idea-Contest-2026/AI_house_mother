...
かつての学生寮には「寮母さん」がいました。ご飯を渡しながら「今日どうだった？」と声を
かけ、体調が悪いと近所の病院を教えてくれ、他の学生が何をしているかも自然と伝えてくれる。
何気ないやりとりの中で、学生の生活を支え、学生と地域をつないでいた存在です。

現代の学生マンションではこの役割が失われました。管理会社は物件を貸すだけで、学生は
入居後の生活・地域理解・キャリア形成を一人で組み立てなければなりません。その結果、
初めての一人暮らしの学生は「困りごとを相談できない」「何に取り組めばよいかわからない」
という孤独に直面しています。

AI 寮母は、この失われた「気軽に相談できて、地域に詳しく、他の学生の取り組みも知って
いる存在」を **LINE Bot + LLM で再現するプロダクト** です。

## 解こうとした 2 つの課題

学生（と保護者）が抱える不安を 2 つに整理しました。以下の数字は発表資料で引用した各種
調査に基づきます（詳細な出典は当日の配布資料に掲載）。

### ① 一人暮らしの孤独と生活不安

- 初めての一人暮らしで「発熱したとき、どこの病院に行けばよいか」「近所で買い物・食事は
  どこがよいか」といった、汎用的な検索では解決しづらい局所的な悩みが積み重なる。
- 一人暮らしの子どもを持つ親の **約 7 割** が、子どもの生活面に不安を抱えている。

### ② やりたいことがわからない・目標が見つからない

- 大学卒業後のキャリア方向性が「決まっていない」学生は **約 6 割**（26・27 年卒対象調査
  n=1,022）。
- 定めきれない理由は、「これだと思うものに出会えていない」（47.5%）、「やり方が分からない」
  （32.2%）、「複数の仕事・キャリアに興味関心がある」（19.5%）が上位を占める。
- 「情報が足りない」というより、**地域・自分・他の学生の取り組みが接続されていない**
  ことに起因する課題である。
...
- **解決アプローチ**: 学生自身が情報提供者となって経験を投稿し、AI がそれを次の学生の相談回答に
  活用する（暗黙知 → 形式知の継承 = **SECI モデル**）。生活相談だけでなく、学生の興味に合わせた
  地域活動を AI 側から提案し、行動のきっかけを生み出す。
- **保護者**: 学生本人が同意した範囲だけを届ける「監視ではなく合意ベースの見守り」を提供。

背景・課題・ビジネスモデルの詳細は [docs/00_product_context.md](docs/00_product_context.md) を参照。

## 汎用 AI（ChatGPT など）との違い

「なぜ ChatGPT で代替できないのか」を 4 観点で整理しました。

| 観点 | 汎用 AI（ChatGPT など） | AI 寮母 |
| --- | --- | --- |
| **情報の範囲** | 浅く広く（全世界の一般的な知識） | その地域・そのマンションに特化した局所知 |
| **情報源** | 学習時点までの汎用データ | 先輩入居者の実体験投稿 + 実在の地域情報（seed） |
| **正確性** | 学習データが古い / 誤情報のリスク | seed は随時更新、実在情報には情報鮮度注記を自動付与 |
| **やり取りの方向性** | 利用者からの一方通行 | AI 側からの寄り添い・地域イベントの Push 提案（双方向） |

技術的にどう担保しているか:

- **「その地域・そのマンションに特化」**: seed データ（地域 30 / イベント 10 / 店舗 15 / 先輩投稿）
  を `context_search` で検索し、Gemini への system prompt に注入することで LLM の生成を
  局所知識で制約している。
- **「情報鮮度と誤情報の抑制」**: 実在情報を回答に含める際は `[情報鮮度: 2026-07]` 注記を自動
  付与し、医療・法律・緊急対応は断定を避けて公式窓口へ誘導する（Truth 制約 NFR-Truth-1〜4、
  詳細は [docs/06_ai_spec.md](docs/06_ai_spec.md)）。
- **「双方向の寄り添い」**: `reply` だけでなく `push` チャネルを使い、興味プロフィールに合う
  地域イベントを AI 側から提案する（「やりたいこと相談」機能）。
